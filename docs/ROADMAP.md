...
## 1.1 Dynamic Today Mission

- [ ] Remove dependency on static `today` fixture data for the live Today experience.
- [ ] Derive today's mission from the active training plan and actual date.
- [ ] Derive workout/activity status from real completion records.
- [ ] Make protein and step goals configurable rather than hard-coded fixture values.
- [ ] Clearly distinguish required and optional activities.
- [ ] Handle rest/recovery days intentionally.
- [ ] Handle days with no active training plan.

**Done when:** every item shown under Today's Mission is real, current, and actionable.

## 1.2 Real Weekly Progress

- [ ] Replace static weekly progress values.
- [ ] Calculate required training activities scheduled this week.
- [ ] Calculate completed required activities.
- [ ] Show strength-session adherence.
- [ ] Show overall plan adherence.
- [ ] Surface whether the current week is on track to Advance, Advance With Warning, or Hold.
- [ ] Do not show protein/step streak data until those data sources are real.

**Done when:** the weekly card accurately reflects the same completion data used by automatic progression.
...
Fitness OS remains private by default. Social features operate through purpose-built shared data rather than exposing raw `fitness_os_data`.
...
3. **Reset Plan UX**
4. **Guide integration with live training/readiness**
5. **Backup/home and shortened workout system**
6. **Strength progression improvements**
7. **Adaptive weekly programming**
8. **Body-composition/goal tracking**
9. **Nutrition and step adherence**
10. **Weekly review and progress improvements**
