...
test("row, shoulder press, and calf raise alternatives expose full How-To guidance", () => {
  for (const id of ["cable-row", "dumbbell-shoulder-press", "calf-raise"]) {
    const guidance = exerciseLibrary.find((exercise) => exercise.id === id)?.guidance;
    assert.ok(guidance, `${id} needs How-To guidance`);
    assert.ok(guidance.setup.length >= 2);
    assert.ok(guidance.execution.length >= 2);
...
test("barbell bench, squat, and RDL expose full How-To guidance", () => {
  for (const id of ["barbell-bench-press", "barbell-squat", "romanian-deadlift"]) {
    const guidance = exerciseLibrary.find((exercise) => exercise.id === id)?.guidance;
    assert.ok(guidance, `${id} needs How-To guidance`);
    assert.ok(guidance.setup.length >= 2);
    assert.ok(guidance.execution.length >= 2);
...
test("all three chest-fly variants expose full canonical How-To guidance", () => {
  for (const id of ["chest-fly", "cable-fly", "dumbbell-fly"]) {
    const guidance = exerciseLibrary.find((exercise) => exercise.id === id)?.guidance;
    assert.ok(guidance, `${id} needs How-To guidance`);
    assert.ok(guidance.setup.length >= 2);
    assert.ok(guidance.execution.length >= 2);
...
test("plank and leg-raise core exercises expose full How-To guidance", () => {
  for (const exerciseId of ["plank", "hanging-leg-raise", "lying-leg-raise"]) {
